...
выражение
'''
__all__ = ['riddle', 'save_results', 'show_results', 'storage']
...
def show_results():
    res = (f'Загадку "{puzzle}" разгадали с {nn}й попытки' 
        if nn > 0 else f'Загадку "{puzzle}" не разгадали' for puzzle, nn in _data.items())
    print(*res, sep='\n')
